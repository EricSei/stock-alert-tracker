...
const CheckoutForm = ({ productSelected, customer }) => {
  const { isAuth, updateUser } = useContext(AuthContext);
  const { accountInformation, setAccountInformation } = useContext(
    StripeContext
  );
  ...
  console.log("productSelected.name", productSelected.name);
  let subscribe = productSelected.name;
  ...
  const handleSubscribe = (subscribe) => {
    ...
    axios({
      method: "PUT",
      url: `${process.env.REACT_APP_API}/createSubscribes`,
      headers: {
        Authorization: `Bearer ${token}`,
      },
      data: { id: isAuth()._id, subscribe: subscribe },
    })
      .then((result) => {
